...
## 8. OpenCode 2.0.6 原生 Code Mode 协作

OpenCode2 与 V1 共用包名 `@ephemushroom/opencode-hermes`（V2 使用 `plugins` 配置）。
插件只通过 `session.model.request` 上报第 1 节的上下文头。原生身份使用
`opencode/<channel>/2.0.6/<app>` UA，以及 `x-opencode-project/session/client`；插件不覆盖它们。

### 工具闭环与所有权

1. OpenCode 原生声明直接工具及 `execute(code: string)`，隐藏工具由 Code Mode 目录提供。
2. Hermes 依据原生身份和 execute schema 选择 `NativeBridge`，统一维护上游别名与 eager/deferred 分类。
3. 上游 ToolSearch 被网关转换为原生 execute 调用。直接工具 schema 来自当前请求；隐藏工具通过原生 `search(...)` 查询。
4. OpenCode 返回执行结果，Hermes 恢复上游 ToolSearch 的 ID、输入及结果；后续调用使用直接工具或 Code Mode 的精确路径。

插件不注册 ToolSearch、不缓存工具目录、不调用 `session.prompt` 投递结果。
旧的 `x-hermes-client-family`、`x-hermes-tool-protocol-version`、`x-hermes-tool-delivery` 不再发送；
`toolSearchDelivery` 旧配置会被忽略。这样网关调整 Grep/Glob 的加载策略时不再需要同步第二份分类表。

部署顺序：先升级 Hermes 至 v0.4.8 或更高版本，再升级插件至 v0.3.0 并重启 OpenCode 服务。旧版插件仍会触发网关的
LegacyCompanion 兼容路径，因此需确认新请求已不含插件 ToolSearch。Code Mode 的路径与签名
保持原样，不能从 TypeScript 签名猜测独立工具的 JSON Schema。
